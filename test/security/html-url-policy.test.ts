...
describe('html URL policy', () => {
  ...
  it('rejects unsafe data documents but allows image data URLs', () => {
    ...
    expect(sanitizeAttrs({ src: 'data:image/png;base64,iVBORw0KGgo=' }).src).toBe('data:image/png;base64,iVBORw0KGgo=')
  })

  it('rejects unsafe srcset candidates', () => {
    expect(sanitizeAttrs({ srcset: 'cover.png 1x, javascript:alert(1) 2x' }).srcset).toBeUndefined()
    expect(sanitizeAttrs({ srcset: 'cover.png 1x, cover@2x.png 2x' }).srcset).toBe('cover.png 1x, cover@2x.png 2x')
  })
})
