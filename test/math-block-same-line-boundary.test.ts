...
\\] where $z$ remains.`
  ...
  it('resets once when appending after an already completed tolerant boundary', () => {
    const md = getMarkdown('stream-math-boundary-append-after-completed-boundary')
    ;(md as any).stream.reset()
    ;(md as any).stream.resetStats()
    ...
    const firstComplete = [
      `First display ${dollar}`,
      'a = 1',
      `${dollar} after first $a$.`,
    ].join('\n')
    ...
    const appendedPartial = [
      firstComplete,
      '',
      `Second display ${dollar}`,
      'b = 2',
    ].join('\n')

    nodes = parseMarkdownToStructure(appendedPartial, md, {
      final: false,
      streamParse: true,
    }) as any[]
    ...
    const appendedComplete = [
      appendedPartial,
      `${dollar} after second $b$.`,
    ].join('\n')

    nodes = parseMarkdownToStructure(appendedComplete, md, {
      final: false,
      streamParse: true,
    }) as any[]
...
$$ where $x$ follows.`, { __markstreamFinal: true }) as any[]
  ...
  it('does not repeatedly reset stream cache for prose-only unresolved tolerant tails', () => {
    const md = getMarkdown('stream-prose-only-unresolved-tolerant-tail-no-reset-loop')
    ;(md as any).stream.reset()
    ;(md as any).stream.resetStats()
  ...
  it('does not reparse indented list tolerant $$ close line as a second math_block', () => {
    const md = getMarkdown('stream-list-tolerant-dollar-close-line-not-new-opener')
    ;(md as any).stream.reset()
    ;(md as any).stream.resetStats()
