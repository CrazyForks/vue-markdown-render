...
| `viewport-priority-options` | `{ rootMargin?: string; heavyBlockMargin?: string; maxTargets?: number }` | `{ rootMargin: '400px', heavyBlockMargin: rootMargin, maxTargets: 640 }` | Advanced observer tuning for `viewportPriority`. `rootMargin` controls node-shell visibility, `heavyBlockMargin` controls heavy child renderers such as Monaco code blocks, and `maxTargets` overrides the auto-disable target count. |
