...
在 VitePress 中，使用内置的 `isDark`（来自 `useData()`）即可与主题切换器同步。库提供了专门用于 VitePress 的 `useDark()` composable：
...
/**
 * VitePress 主题 composable，用于暗黑模式
 * 使用 VitePress 内置的 isDark（来自 useData()）
 */
...
|------|---------------------|-------------------|
| `isDark` | 直接传给 `<CodeBlockNode :is-dark="isDark" />` | 通过 `<MarkdownRender :is-dark="isDark" />` 传入，会自动转发 |
| Theme props | `:dark-theme="theme"` `:light-theme="theme"` | `:code-block-dark-theme="theme"` `:code-block-light-theme="theme"` |

**在独立应用中使用 @vueuse/core：**

对于非 VitePress 的 Vue 应用，使用 `@vueuse/core` 的 `useDark()`：
...
const isDark = useDark() // Ref<boolean>，响应式跟随系统/主题偏好
</script>
