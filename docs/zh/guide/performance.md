...
它会构建 playground，通过 `vite preview` 跑 Diagnostic Studio baseline/thinking/diff/stress 与主 playground reverse-flex chat 场景，并在 `benchmark/` 下生成 JSON 与 Markdown 报告，包含环境披露、LCP、CLS、settle time、p95 `requestAnimationFrame` interval、long task、DOM 节点数、fallback、重节点 readiness、滚动漂移和 Chrome-only best-effort 的 component unmount + GC 后 heap 等指标。1 MB、1000 code blocks、100 Mermaid、10k nodes 这类 synthetic 场景属于后续 1.0.x 覆盖，未接入脚本前不要作为 1.0 release evidence。
