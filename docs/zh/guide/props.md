...
| `viewport-priority-options` | `{ rootMargin?: string; heavyBlockMargin?: string; maxTargets?: number }` | `{ rootMargin: '400px', heavyBlockMargin: rootMargin, maxTargets: 640 }` | `viewportPriority` 的高级 observer 调参。`rootMargin` 控制节点外壳可见性，`heavyBlockMargin` 控制 Monaco code block 等重型子渲染器，`maxTargets` 覆盖自动关闭的 observer 数量上限。 |
