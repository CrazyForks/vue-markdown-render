...
| `virtual-scroll` | `MarkstreamVirtualScrollOptions` | – | 开启宿主 virtual-scroll 协作。外层 message/thread virtualizer 应使用回调里的逻辑 `totalHeight`、restore state 和 settle/final 事件，而不是读取当前 DOM 高度。 |
...
| `virtual-scroll` | – | 向外层 virtualizer 报告逻辑高度与恢复状态。监听 `height-change`，并使用 `metrics.totalHeight` 作为消息/item 高度。 |
