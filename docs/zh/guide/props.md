...
| `parse-coalesce-ms` | `number` | `80` | 1.x 可用的高级性能调参项：smooth streaming 合并字符级更新时的最小解析间隔。设为 `0` 可让每次 smooth-stream commit 都触发解析。默认调度语义后续可能继续优化。 |
...
可用字段：`minCharsPerSecond`、`maxCharsPerSecond`、`targetLatencyMs`、`catchUpLatencyMs`、`catchUpThreshold`、`maxCommitFps`、`startDelayMs`、`maxCharsPerCommit`、`flushOnFinish`。这些选项在渲染器创建时一次性读取；如需动态切换，请更改组件 `key`。只有在有明确性能测量需求时，再使用 `parse-coalesce-ms` 单独调整解析合并间隔。
