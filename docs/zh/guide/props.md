...
| `parse-coalesce-ms` | `number` | `80` | smooth streaming 合并字符级更新时的最小解析间隔。设为 `0` 可让每次 smooth-stream commit 都触发解析。 |
...
可用字段：`minCharsPerSecond`、`maxCharsPerSecond`、`targetLatencyMs`、`catchUpLatencyMs`、`catchUpThreshold`、`maxCommitFps`、`startDelayMs`、`maxCharsPerCommit`、`flushOnFinish`。这些选项在渲染器创建时一次性读取；如需动态切换，请更改组件 `key`。如需单独调整解析合并间隔，请使用 `parse-coalesce-ms`。
