...
HTML preview 默认使用 `sandbox=""`，因此不可信 HTML / SVG 预览不会默认执行脚本，也不会继承宿主页面 origin。只有在可信 demo 场景下才建议显式开启 `htmlPreviewAllowScripts`；对于不可信预览内容，不要把 `allow-scripts` 和 `allow-same-origin` 组合在一起。
